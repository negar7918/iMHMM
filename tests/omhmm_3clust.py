...
import time
import warnings
from iMHMM import maximizer, rand_initialization
...
class RegulizedGaussianHMM(hmm.GaussianHMM):
    ...
    def __init__(self, similarity, epsilon, other_trans, hyperparam, *args, **kwargs):
    ...
    def regularize_trans(self, a, hyperparam, epsilon):
        return maximizer.maximize_multi_clusters(a, np.multiply(self.other_trans, hyperparam), epsilon)
...
class SpaMHMM(BaseEstimator):
    def __init__(self, epsilon, hyperparam, n_nodes, mix_dim, n_components, n_features, graph=None,
    ...
    def init_params(self, X):
        ...
        self.mixCoef = np.reshape(np.ones(3) * (1/3), (1, 3))
        ...
        if self.emission == 'gaussian':
            self.mixModels = [RegulizedGaussianHMM(n_components=self.n_components,
                                                   ...
                                                   other_trans=np.zeros((self.n_components, self.n_components)),
                                                   hyperparam = self.hyperparam, epsilon=self.epsilon)
                              for i in range(self.mix_dim)]

            for m in range(self.mix_dim):
                self.mixModels[m]._init(X)
                seed = 1
                trans = rand_initialization.generate_transitions_random(seed, 3, self.n_components, .1)
    ...
    def fit(self, X, y, lengths=None, valid_data=None):
        ...
        for it in range(self.n_iter):
            ...
            self.mixModels[0].other_trans = [self.mixModels[1].transmat_, self.mixModels[0].transmat_]
            self.mixModels[1].other_trans = [self.mixModels[0].transmat_, self.mixModels[2].transmat_]
            self.mixModels[2].other_trans = [self.mixModels[0].transmat_, self.mixModels[1].transmat_]
            ...
            self.mixModels[2].other_trans = [self.mixModels[0].transmat_, self.mixModels[1].transmat_]
            self.mixModels[2]._do_mstep(stats['mix_idx' + str(2)])
            ####

            print('trans0 {}'.format(self.mixModels[0].transmat_))
            print('trans1 {}'.format(self.mixModels[1].transmat_))
            print('trans2 {}'.format(self.mixModels[2].transmat_))

            print('entropy: {}'.format(calculate_entropy(self.mixModels[0].transmat_)))
            print('entropy: {}'.format(calculate_entropy(self.mixModels[1].transmat_)))
            print('entropy: {}'.format(calculate_entropy(self.mixModels[2].transmat_)))
