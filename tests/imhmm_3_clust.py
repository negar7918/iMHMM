...
import time
import warnings
from iMHMM import rand_initialization
...
class RegulizedGaussianHMM(hmm.GaussianHMM):
    ...
    def __init__(self, similarity, other_trans, length, *args, **kwargs):
    ...
    def regularize_trans(self, a):
        J = a.shape[0]
        new_a = np.empty((J, J))
        for s in range(J):
            denom = self.length + np.sum(a[s, :])
            max_prob = np.max(a[s, :])
            for k in range(J):
                mass = self.length if a[s, k] == max_prob else 0
                new_a[s, k] = ((mass) + a[s, k]) / denom
        self.normalize(new_a, axis=1)
        self.normalize(a, axis=1)
        if calculate_entropy(new_a) > calculate_entropy(a):
            return a
        else:
            return new_a
    ...
    def _do_mstep(self, stats):
        ...
        if 't' in self.params:
            ...
            self.transmat_ = self.regularize_trans(self.transmat_)
            print('entropy: {}'.format(calculate_entropy(self.transmat_)))
...
class SpaMHMM(BaseEstimator):
    def __init__(self, length, n_nodes, mix_dim, n_components, n_features, graph=None,
    ...
    def init_params(self, X):
        ...
        self.mixCoef = np.reshape(np.array([1/3, 1/3, 1/3]), (1, 3))
        ...
        if self.emission == 'gaussian':
            self.mixModels = [RegulizedGaussianHMM(n_components=self.n_components,
                                              ...
                                              other_trans=np.zeros((self.n_components, self.n_components)), length=self.length)
    ...
    def fit(self, X, y, lengths=None, valid_data=None):
        ...
        for it in range(self.n_iter):
            ...
            self.mixModels[0].similarity = similarity
            self.mixModels[1].similarity = similarity
