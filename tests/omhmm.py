...
class RegulizedGaussianHMM(hmm.GaussianHMM):
    ...
    def __init__(self, similarity, other_trans, epsilon, *args, **kwargs):
    ...
    def regularize_trans(self, a):
        return maximizer.maximize_new(a, self.other_trans * 1, self.epsilon)
...
class SpaMHMM(BaseEstimator):
    def __init__(self, epsilon, n_nodes, mix_dim, n_components, n_features, graph=None,
    ...
    def init_params(self, X):
        ...
        if self.emission == 'gaussian':
            self.mixModels = [RegulizedGaussianHMM(n_components=self.n_components,
                                              ...
                                              other_trans=np.zeros((self.n_components, self.n_components)), epsilon=self.epsilon)
